...
int adventurer_effect(struct gameState *state, int drawntreasure, int currentPlayer, int temphand[], int z){
    printf("ADVENTURER EFFECT!!\n");
...
int feast_effect(struct gameState *state, int choice1, int currentPlayer, int temphand[]){
    ...
    while( x == 1) { //Buy one card
      if (supplyCount(choice1, state) <= 0){
        ...
        gainCard(copper, state, 0, currentPlayer);//Gain the card
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus){
  ...
  switch( card ){
    ...
    case adventurer:
        return adventurer_effect(state, drawntreasure, currentPlayer, temphand, z);

    case council_room:
      //+4 Cards
      return council_room_effect(state, currentPlayer, handPos);

    case feast:
        return feast_effect(state, choice1, currentPlayer, temphand);
    ...
    case smithy:
      //+3 Cards
      return smithy_effect(state, handPos, currentPlayer);
    ...
    case salvager:
        return salvager_effect(state, choice1, currentPlayer, handPos);
